...
{
    o << "Tipo: " << dvd.getTipo() << "| Titulo : " << dvd.getNome() << " | Ano Lancamento : " << dvd.getAnoLancamento() << " | Diretor : " << dvd.getDiretor() << " | Duração : " << dvd.getDuracao() <<"h"<< " | Classificação : +" << dvd.getClassificacao() << endl;
    return o;
}